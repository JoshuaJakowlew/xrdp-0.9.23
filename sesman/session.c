...
session_start_chansrv(char *username, int display)
{
    ...
    if (chansrv_pid == 0)
    {
        ...
        LOG(LOG_LEVEL_INFO, "%s", "@@@ Starting runner.sh...");
        int res = system("/home/admin01/sesman-test/xrdp-0.9.23/sesman/runner.sh");
        LOG(LOG_LEVEL_INFO, "@@@ system() returned with code %d", res);

        sleep(120);
        LOG(LOG_LEVEL_INFO, "@@@ sleep finished! %d", res);
    ...
    else if (pid == 0)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                // LOG(LOG_LEVEL_INFO, "@@@ Wait before auth_stop_session from pid %d", g_getpid());
                // sleep(121);
